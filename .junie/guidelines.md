Project: floss-funding.dev — Development Guidelines (for advanced contributors)
...
- Ruby and Bundler
  - Runtime is intended to run on the latest release of Ruby (>= 3.4.5 at time of this writing).
...
- Running tests (verified)
  ...
  - Focused runs
    ...
    - Example: K_SOUP_COV_MIN_HARD=false bin/rspec spec/path/to/file_spec.rb:42
  - Output visibility
  ...
  - During a spec run, the presence of output about missing activation keys is not a concern. These are harmless nags.
- Adding new tests (guidelines)
  ...
  - Place new specs under spec/ mirroring app/ structure where possible. Require "rails_helper" at the top of spec files that need Rails. There is no need to require "spec_helper", as it is automatically loaded by .rspec.
  - If your code relies on environment variables that drive behavior test using rspec-stubbed_env:
...
- Code style and static analysis
  ...
  - Reek is configured to scan {app,spec,tests}/**/*.rb. Use:
...
- Documentation
  - Generate YARD docs with: bundle exec rake yard. It includes {app/lib}/**/*.rb and extra docs like README.md, CHANGELOG.md, RUBOCOP.md, REEK, etc.
